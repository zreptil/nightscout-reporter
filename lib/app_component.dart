  ...
  implements OnInit
{
  ...
  {
    if (ret != "stop" && reportData != null && g.period.shiftStartBy(shift) == reportData.begDate &&
...
// */
  ...
  Future<ReportData> loadData(bool isForThumbs)
  async {
...
// */
    ...
    url = "${data.user.apiUrl}treatments.json?find[created_at][\$gte]=${begDate.year -
      1}-01-01T00:00:00.000Z&find[eventType]=Profile Switch";
    ...
    while (begDate <= endDate)
    {
      ...
      if (g.period.isDowActive(begDate.weekday - 1))
      {
        ...
        if (lastTempBasal == null)
        {
          ...
          url = "${data.user.apiUrl}treatments.json?find[created_at][\$lt]=${profileBeg
        ...
        url = "${data.user.apiUrl}treatments.json?find[created_at][\$gte]=${profileBeg
...
//*
  ...
  void createPDF({bool isForThumbs: false})
  {
    ...
    display("");
    pdfList.clear();
    loadData(isForThumbs).then((ReportData src)
    async {
