...
class PrintDailyAnalysis extends BaseDaily {
  ...
  @override
  dynamic get estimatePageCount => {"count": g?.period?.dayCount ?? 0, "isEstimated": false};
  ...
  double basalY(double value) => profMax != 0 && value != null ? graphHeight - (basalHeight / profMax * value) : 0.0;
...
//    glucMax = gridLines * 50.0;
    ...
    ProfileGlucData profile = repData.profile(date);
    double yHigh = glucY(math.min(glucMax, repData.status.settings.thresholds.bgTargetTop.toDouble()));
    double yLow = glucY(repData.status.settings.thresholds.bgTargetBottom.toDouble());
    List targetValues = [];
    double lastTarget = -1;
    ...
    profMax = -1000.0;
    for (ProfileEntryData entry in day.profile) profMax = math.max(entry.tempAdjusted ?? 0, profMax);
...
//      "fillOpacity": opacity
    ...
    _graphLines["stack"].add({
      ...
      "canvas": [
        {"type": "polyline", "lineWidth": cm(lw), "closePath": true, "lineColor": color, "points": areaPoints}
      ]
    });
