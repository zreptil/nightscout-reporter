...
//    bloody.add(entry);
  ...
  CalcIOBData iob(ReportData data, DateTime time, DayData yesterday) {
    ...
    var check = time.millisecondsSinceEpoch - data.maxInsulinEffectInMS;
    ...
    for (var t in list) {
      if (t.createdAt.millisecondsSinceEpoch < check) continue;   // die Treatments, deren Wirksamkeit auf jeden Fall zum Zeitpunkt "time" abgelaufen ist, ignorieren
      if (t.createdAt.millisecondsSinceEpoch > time.millisecondsSinceEpoch) continue;   // die Treatments, die zum Zeitpunkt "time" noch gar nicht drin sind, ignorieren      var tIOB = t.calcIOB(profile, time);
